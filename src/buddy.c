...
static buddy_alloc_t buddy_allocs[BUDDY_DEP];
...
void init_buddy_alloc()
{
    ...
    for(size_t i = 0; i < BUDDY_DEP; ++i, list_size >>= 1)
    {
        ...
        buddy_allocs[i].list_start = (buddy_list_node_t*) kgetpersistedmem(list_size * sizeof(buddy_list_node_t));
    ...
    buddy_allocs[BUDDY_DEP - 1].map[lb_size / 32] |= (1 << (lb_size % 32));
...
void free_address_page(void* p, size_t n)
{
    ...
    pos /= (1 << n);
    buddy_allocs[n].map[pos / 32] ^= (1 << (pos % 32));
    if(buddy_allocs[n].map[pos / 32] & (1 << (pos % 32)) || n == BUDDY_DEP - 1)
    {
        buddy_allocs[n].list_start[pos].next = buddy_allocs[n].list_head;
        buddy_allocs[n].list_start[pos].prev = NULL;
        if(buddy_allocs[n].list_head != NULL)
