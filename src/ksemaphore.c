...
void ksemaphore_up(struct ksemaphore_desc_t* sem, size_t num)
{
    ...
    while(1)
    {
        if(sem->head.next == &(sem->head))
            return;
        struct process_info_t* proc = parentof(sem->head.next, struct process_info_t, semaphore_node.node);
        if(proc->semaphore_node.demand > sem->surplus)
            return;
        kwakeup(proc);
        in_sched_queue(proc);
        circular_list_remove(&(proc->semaphore_node.node));
        sem->surplus -= proc->semaphore_node.demand;
        proc->semaphore_node.demand = 0;
    }
}
...
int ksemaphore_down(struct ksemaphore_desc_t* sem, size_t demand, int timeout)
{
    ...
    cur_process->semaphore_node.demand = demand;
    circular_list_insert(sem->head.pre, &(cur_process->semaphore_node.node));
    ...
    if(timeout > 0)
        ...
        kwait(cur_process);
    schedule();
    if(cur_process->semaphore_node.demand == 0)
        return demand;
